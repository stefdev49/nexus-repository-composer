    ...
    extends ContentProxyFacetSupport
{
  ...
  protected Content getCachedContent(final Context context) throws IOException {
    ...
    switch (assetKind) {
      case PACKAGES:
        return content().get(PACKAGES_JSON).get();
      case LIST:
        return content().get(LIST_JSON).get();
      case PROVIDER:
        return content().get(buildProviderPath(context)).get();
      case PACKAGE:
        return content().get(buildPackagePath(context)).get();
      case ZIPBALL:
        return content().get(buildZipballPath(context)).get();
      default:
