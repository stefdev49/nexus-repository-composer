...
/**
 * Programmatic upload handler for Composer hosted repositories.
 */
@Named
@Singleton
  ...
  public Response handle(@Nonnull final Context context) throws Exception {
    ...
    if (request.isMultipart() && request.getMultiparts() != null) {
      for (PartPayload part : request.getMultiparts()) {
        ...
        if (ComposerJsonProcessor.SOURCE_TYPE_FIELD_NAME.equals(part.getFieldName())) {
          sourceType = checkNotNull(readPartStreamToString(part));
        } else if (ComposerJsonProcessor.SOURCE_URL_FIELD_NAME.equals(part.getFieldName())) {
          sourceUrl = checkNotNull(readPartStreamToString(part));
        } else if (ComposerJsonProcessor.SOURCE_REFERENCE_FIELD_NAME.equals(part.getFieldName())) {
          sourceRef = checkNotNull(readPartStreamToString(part));
        } else if (ComposerJsonProcessor.PACKAGE_FIELD_NAME.equals(part.getFieldName())) {
