 ...
 */
package org.sonatype.nexus.repository.composer.datastore.internal.hosted.metadata;
...
import org.sonatype.nexus.repository.composer.datastore.internal.hosted.AssetChange;
...
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.BZ2;
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.DEB;
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.GZ;
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.P_ARCHITECTURE;
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.P_INDEX_SECTION;
import static org.sonatype.nexus.repository.composer.internal.ComposerProperties.P_PACKAGE_NAME;
...
/**
 * Composer metadata facet. Holds the logic for metadata recalculation.
 */
@Named(ComposerFormat.NAME)
@Exposed
public class ComposerHostedMetadataFacet
    extends FacetSupport
{
  ...
  @Inject
  public ComposerHostedMetadataFacet(
      final ObjectMapper mapper,
      final Clock clock,
      final Cooperation2Factory cooperationFactory,
      @Named("${nexus.composer.metadata.cooperation.enabled:-true}") final boolean cooperationEnabled,
      @Named("${nexus.composer.metadata.cooperation.majorTimeout:-0s}") final Duration majorTimeout,
      @Named("${nexus.composer.metadata.cooperation.minorTimeout:-30s}") final Duration minorTimeout,
      @Named("${nexus.composer.metadata.cooperation.threadsPerKey:-100}") final int threadsPerKey)
  {
  ...
  private Content doRebuildMetadata(final List<AssetChange> changeList) throws IOException {
    ...
    try (CompressingTempFileStore store = buildPackageIndexes(changeList)) {
      for (Map.Entry<String, CompressingTempFileStore.FileMetadata> entry : store.getFiles().entrySet()) {
        FluentAsset metadataAsset = composerFacet.put(
            packageIndexName(entry.getKey(), StringUtils.EMPTY),
            new StreamPayload(entry.getValue().plainSupplier(), entry.getValue().plainSize(), ComposerMimeTypes.TEXT)
        );
        ...
        FluentAsset gzMetadataAsset = composerFacet.put(
            packageIndexName(entry.getKey(), GZ),
            new StreamPayload(entry.getValue().gzSupplier(), entry.getValue().bzSize(), ComposerMimeTypes.GZIP)
        );
        ...
        FluentAsset bzMetadataAsset = composerFacet.put(
            packageIndexName(entry.getKey(), BZ2),
            new StreamPayload(entry.getValue().bzSupplier(), entry.getValue().bzSize(), ComposerMimeTypes.BZIP)
        );
  ...
  private ComposerContentFacet content() {
    return facet(ComposerContentFacet.class);
  }
