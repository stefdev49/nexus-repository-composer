...
@Named
@Singleton
public class ComposerHostedHandler
    ...
    implements Handler
{
  ...
  public Response handle(@Nonnull final Context context) throws Exception {
    ...
    ComposerContentFacet contentFacet = context.getRepository().facet(ComposerContentFacet.class);
  ...
  private Response doGet(
      ...
      final String path,
      final ComposerContentFacet contentFacet) throws IOException
  {
    if (isMetadataRebuildRequired(path, contentFacet)) {
      context.getRepository().facet(ComposerHostedFacet.class).rebuildMetadata();
    }
  ...
  private Response doPost(final Context context,
                          ...
                          final ComposerContentFacet contentFacet) throws IOException
  {
    final ComposerHostedFacet hostedFacet = context.getRepository().facet(ComposerHostedFacet.class);
    if ("rebuild-indexes".equals(path)) {
      ...
      try (TempBlob tempBlob = contentFacet.getTempBlob(payload)) {
        ControlFile controlFile = ComposerPackageParser
            ...
            .getControlFile();
        String assetPath = ComposerFacetHelper.buildAssetPath(controlFile);
  ...
  private boolean isMetadataRebuildRequired(final String path, final ComposerContentFacet contentFacet)
  {
  ...
  private String assetPath(final Context context) {
    return context.getAttributes().require(ComposerSnapshotHandler.State.class).assetPath;
  }
}
