 ...
 */
package org.sonatype.nexus.repository.composer.orient.internal.snapshot;
...
import org.sonatype.nexus.repository.composer.orient.internal.OrientFacetHelper;
 ...
 * @since 3.17
 */
public abstract class OrientComposerSnapshotFacetSupport
  ...
  protected void createSnapshot(final String id, final Iterable<SnapshotItem> snapshots) throws IOException {
    ...
    for (SnapshotItem item : snapshots) {
      ...
      try (final TempBlob streamSupplier = storageFacet
          .createTempBlob(item.content.openInputStream(), ComposerFacetHelper.hashAlgorithms)) {
        AssetBlob blob = tx.createBlob(item.specifier.path, streamSupplier, ComposerFacetHelper.hashAlgorithms, null,
