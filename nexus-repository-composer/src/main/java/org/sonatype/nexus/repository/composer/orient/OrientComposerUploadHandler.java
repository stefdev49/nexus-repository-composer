 ...
 */
package org.sonatype.nexus.repository.composer.orient;
...
import org.sonatype.nexus.repository.composer.orient.internal.hosted.OrientComposerHostedFacet;
...
{
  @Inject
  public OrientComposerUploadHandler(@Named("simple") final VariableResolverAdapter variableResolverAdapter,
  ...
  public UploadResponse handle(final Repository repository, final ComponentUpload upload) throws IOException {
    ...
    try (TempBlob tempBlob = storageFacet
        .createTempBlob(upload.getAssetUploads().get(0).getPayload(), ComposerFacetHelper.hashAlgorithms)) {
      ControlFile controlFile = ComposerPackageParser.parsePackageInfo(tempBlob).getControlFile();
