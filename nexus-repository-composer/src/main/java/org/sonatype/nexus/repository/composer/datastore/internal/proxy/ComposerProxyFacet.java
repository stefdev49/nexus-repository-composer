    ...
    extends ContentProxyFacetSupport
{
  ...
  protected Content getCachedContent(final Context context) {
    return facet(ComposerContentFacet.class).get(assetPath(context)).orElse(null);
  }
  ...
  protected Content store(final Context context, final Content content) throws IOException {
    ...
    return facet(ComposerContentFacet.class).put(assetPath(context), content).markAsCached(content).download();
  }
  ...
  private Optional<SnapshotItem> fetchLatest(final ContentSpecifier spec) throws IOException {
    ...
    Content oldVersion = facet(ComposerContentFacet.class).get(spec.path).orElse(null);
    ...
    if (status.getStatusCode() == HttpStatus.SC_OK) {
      ...
      Content storedContent = facet(ComposerContentFacet.class).put(spec.path, fetchedContent).download();
  ...
  protected void doIndicateVerified(final Content content, final CacheInfo cacheInfo, final String assetPath) {
    ...
    facet(ComposerContentFacet.class).getAsset(assetPath)
