...
@Named
@Singleton
public class OrientComposerHostedHandler
    ...
    implements Handler
{
  ...
  public Response handle(final Context context) throws Exception {
    ...
    OrientComposerFacet composerFacet = context.getRepository().facet(OrientComposerFacet.class);
    OrientComposerHostedFacet hostedFacet = context.getRepository().facet(OrientComposerHostedFacet.class);

    switch (method) {
      case GET:
      case HEAD:
        return doGet(path, composerFacet);
      case POST:
  ...
  private Response doPost(final Context context,
                          final String path,
                          final String method,
                          final OrientComposerHostedFacet hostedFacet) throws IOException
  {
  ...
  private Response doGet(final String path, final OrientComposerFacet composerFacet) throws IOException {
    Optional<Content> content = composerFacet.get(path);
